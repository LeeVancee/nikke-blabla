...
const NikkeButton = ({ type, content, style, onClick }: NikkeButtonProps) => {
  return (
    <div className={styles.btn} style={style} onClick={onClick}>
      <span
        className={`${styles.value} ${
          type === buttonType.Cancel ? 'valueExit' : ''
        }`}
      >
        {content === undefined ? '按钮' : content}
      </span>
      {type === buttonType.Success && (
        <Image src="/btn_bg.png" width={150} height={50} alt="" />
      )}
      {type === buttonType.Cancel && (
        <Image
          src="/btn_white_bg.png"
          width={150}
          height={50}
          alt=""
          priority
